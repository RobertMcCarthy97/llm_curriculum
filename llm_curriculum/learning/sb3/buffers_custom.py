...
class LLMBasicReplayBuffer(ReplayBuffer):
    ...
    def __init__(
        ...
        if do_parent_relabel:
            self.obs_parent_goals = np.zeros(
                (
                    self.buffer_size,
                    self.n_envs,
                    observation_space["desired_goal"].shape[0],
                ),
                dtype=observation_space["desired_goal"].dtype,
            )
            self.parent_goal_rewards = np.zeros(
    ...
    def add(
        ...
        if self.do_parent_relabel:
            self.obs_parent_goals[self.pos] = np.array(
                [info.get("obs_parent_goal", np.nan) for info in infos]
            )
            self.parent_goal_rewards[self.pos] = np.array(
                [info.get("obs_parent_goal_reward", np.nan) for info in infos]
            )
    ...
    def _get_samples(
        ...
        obs_ = {
            key: obs[batch_inds, env_indices, :].copy()
        ...
        if self.do_parent_relabel:
            ...
            prob_indices = np.where(
                np.random.rand(len(valid_idxs)) <= self.parent_relabel_p
            )[0]
            selected_idxs = valid_idxs[prob_indices]
            # relabel
            obs_["desired_goal"][selected_idxs] = obs_parent_goals_[selected_idxs]
            next_obs_["desired_goal"][selected_idxs] = obs_parent_goals_[
                selected_idxs
            ]  # TODO: "Always using obs parent goal for next obs! (for now)"
            rewards_[selected_idxs] = parent_rewards_[
                selected_idxs
            ]  # TODO: include child reward as a mini bonus?
            assert np.all(~np.isnan(rewards_)) and np.all(
                ~np.isnan(obs_["desired_goal"])
            ), "NaNs in rewards or goals!"
        ...
        assert (
            dones.sum() == 0
        ), "Not setup to handle llm terminations yet - assume no terminations for now"
...
class SeparatePoliciesReplayBuffer(BaseBuffer):
        ...
        if len(relations["parents"]) > 0:
            ...
            for parent_name in relations["parents"].keys():
                ...
                if parent_name in self.all_models.keys():
                    ...
                    self.parent_rewards_dict[parent_name] = np.zeros(
                        (self.buffer_size, self.n_envs), dtype=np.float32
                    )
                    # record valid relation
                    self.valid_relations["parents"][parent_name] = relations["parents"][
                        parent_name
                    ]
        ...
        if len(relations["children"]) > 0:
            for child_name in relations["children"].keys():
                if child_name in self.all_models.keys():
                    self.valid_relations["children"][child_name] = relations[
                        "children"
                    ][child_name]
    ...
    def add(
        ...
        assert all(
            [info["prev_task_name"] == self.task_name for info in infos]
        ), "Task name mismatch"
        ...
        if self.has_parent:
            for parent_name in self.valid_relations["parents"].keys():
                self.parent_rewards_dict[parent_name][self.pos] = np.array(
                    [info[f"obs_{parent_name}_reward"] for info in infos]
                )  # TODO: make sure timestep correct on these rewards
    ...
    def _get_samples(
        ...
        if self.optimize_memory_usage:
            unnormed_data["next_obs"] = self.observations[
                (batch_inds + 1) % self.buffer_size, env_indices, :
            ].copy()
        else:
            unnormed_data["next_obs"] = self.next_observations[
                batch_inds, env_indices, :
            ].copy()
        unnormed_data["acts"] = self.actions[batch_inds, env_indices, :].copy()
        unnormed_data["dones"] = self.dones[batch_inds, env_indices].copy() * (
            1 - self.timeouts[batch_inds, env_indices].copy()
        )
    ...
    def _get_child_data(self, batch_size):
        ...
        for child_name in self.valid_relations["children"].keys():
            ...
            child_upper_bound = (
                child_buffer.buffer_size if child_buffer.full else child_buffer.pos
            )
            if child_upper_bound > child_batch_size:
                ...
                child_data = child_buffer.sample_unnormed_for_parent(
                    child_batch_size, self.task_name
                )
    ...
    def sample_unnormed_for_parent(self, batch_size: int, parent_name: str):
        """Custom"""
        assert (
            not self.optimize_memory_usage
        ), "Not implemented for memory efficient variant"
        assert self.has_parent
        assert parent_name in self.valid_relations["parents"].keys()
        ...
        return {
            ...
            "dones": (
                self.dones[batch_inds, env_indices].copy()
                * (1 - self.timeouts[batch_inds, env_indices].copy())
            ),
            "rewards": self.parent_rewards_dict[parent_name][
                batch_inds, env_indices
            ].copy(),
        }
