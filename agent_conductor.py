...
class AgentConductor():
    def __init__(self, env, manual_decompose_p=None, dense_rew_lowest=False, single_task_name=None):
        self.env = env
        self.manual_decompose_p = manual_decompose_p
        self.dense_rew_lowest = dense_rew_lowest
        self.single_task_name = single_task_name
        if self.single_task_name is not None:
    ...
    def decompose_task(self, task):
        if task.name == self.single_task_name:
            return task
    ...
    def record_chosen_task(self, chosen_task, prev_active_task=None, reset=False):
        ...
        if prev_active_task is not None:
            task_changed = (chosen_task.name != prev_active_task.name)
